...
export default async function StudentReportCardPage({
  ...
  if (!student) {
    return (
      <div className="space-y-4">
        <h1 className="text-2xl font-semibold text-gray-900">My Report Card</h1>
  ...
  try {
    reportCard = await reportCardService.getStudentReportCard(student.id, resolvedTermId);
  } catch (error) {
  ...
  return (
    <div className="report-card-page space-y-6 print:space-y-0">
      <div className="report-card-toolbar flex flex-wrap items-center justify-between gap-3 print:hidden">
        <Link
          href={`/dashboard/student?termId=${resolvedTermId}`}
          className="inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
        >
          Back to Dashboard
        </Link>
                    ...
                    reportCard.grades.map((grade) => (
                      <tr key={`${reportCard.student.id}-${grade.subjectId}`}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-700">
                          {formatNumber(grade.caScore)}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-700">
                          {formatNumber(grade.examScore)}
                        </td>
