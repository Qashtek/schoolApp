...
export default async function ParentStudentReportCardPage({
  params,
  searchParams,
}: {
  params: PageParams;
  ...
  // Verify parent-child link before loading any report-card data.
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-sm text-red-700">
          You are not allowed to view this student&apos;s report card.
        </div>
        <Link
          href="/dashboard/parent"
          className="inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
  ...
  const [school, activeTerm] = await prisma.$transaction([
  ...
  const resolvedTermId = requestedTermId || activeTerm?.id || '';
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-dashed border-gray-300 bg-white p-6 text-sm text-gray-600">
          No active term is available yet. Ask your school administrator to activate a term.
        </div>
        <Link
          href="/dashboard/parent"
          className="inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
  ...
  let reportCard: Awaited<ReturnType<ReportCardService['getStudentReportCard']>>;

  try {
    reportCard = await reportCardService.getStudentReportCard(studentId, resolvedTermId);
  } catch (error) {
